...
/**
 * Decrypt a SSESENC1 file on disk without loading the whole ciphertext into RAM.
 * Needed for titles >2 GiB (Node `fs.readFileSync` / Buffer hard limit).
 *
 * @returns {Promise<{ plainBytes: number, compression: number }>}
 */
async function decryptFile(encPath, outPath, key) {
  ...
  try {
    const header = Buffer.alloc(FULL_HEADER);
    // position=null so the fd cursor advances past the header for chunk reads.
    const n = fs.readSync(fdIn, header, 0, FULL_HEADER, null);
    if (n !== FULL_HEADER) throw new Error("encrypted file too short for header");
    const { compression, chunkSize, plainSize } = readHeader(header);

    tmpPayload =
      compression === COMPRESSION_GZIP ? `${outPath}.payload.bin` : null;
    const payloadPath = tmpPayload || outPath;
    fdOut = fs.openSync(payloadPath, "w");
    ...
    while (writeAt < plainSize) {
      const thisLen = Math.min(chunkSize, plainSize - writeAt);
      if (fs.readSync(fdIn, ivBuf, 0, IV_LEN, null) !== IV_LEN) {
        throw new Error("short read (iv) while decrypting");
      }
      const ct = Buffer.alloc(thisLen);
      if (fs.readSync(fdIn, ct, 0, thisLen, null) !== thisLen) {
        throw new Error("short read (ct) while decrypting");
      }
      if (fs.readSync(fdIn, tagBuf, 0, TAG_LEN, null) !== TAG_LEN) {
        throw new Error("short read (tag) while decrypting");
      }
      const decipher = crypto.createDecipheriv("aes-256-gcm", key, ivBuf);
      decipher.setAuthTag(tagBuf);
      const plain = Buffer.concat([decipher.update(ct), decipher.final()]);
      fs.writeSync(fdOut, plain);
      writeAt += plain.length;
    }
    fs.closeSync(fdOut);
    fdOut = null;
    ...
    if (compression === COMPRESSION_GZIP) {
      await pipeline(
        fs.createReadStream(tmpPayload),
        zlib.createGunzip(),
        fs.createWriteStream(outPath),
      );
      fs.rmSync(tmpPayload, { force: true });
      tmpPayload = null;
    }

    const plainBytes = fs.statSync(outPath).size;
    return { plainBytes, compression };
  } finally {
    try {
      fs.closeSync(fdIn);
    } catch {
    ...
    if (tmpPayload) fs.rmSync(tmpPayload, { force: true });
  }
}

/** True if path starts with SSESENC1 magic (reads 8 bytes only). */
function isEncryptedFile(filePath) {
  const fd = fs.openSync(filePath, "r");
  try {
    const magic = Buffer.alloc(8);
    const n = fs.readSync(fd, magic, 0, 8, 0);
    return n === 8 && magic.equals(MAGIC);
  } finally {
    fs.closeSync(fd);
  }
}
