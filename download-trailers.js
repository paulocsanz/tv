...
// bytes ever moving again. requestTimeout is Node's own socket-level *idle*
// timeout (no activity for this long, not "the whole request must finish by
// X"), so it fires on a genuinely dead connection without punishing a
// legitimately slow-but-active upload. throwOnRequestTimeout is required
// for this to actually abort anything - without it, requestTimeout alone
// just logs a warning and never aborts.
...
const s3Client = new S3Client({
  ...
  requestHandler: new NodeHttpHandler({ requestTimeout: UPLOAD_STALL_TIMEOUT_MS, throwOnRequestTimeout: true }),
});
...
async function uploadToS3(filePath, s3Key, label, contentType) {
  ...
  for (let attempt = 1; attempt <= UPLOAD_RETRIES; attempt++) {
    try {
      ...
      // A hard backstop that doesn't depend on the SDK, the socket, or
      // upload.abort() ever noticing anything on its own. Must reset on
      // every progress event, not fire from a flat deadline - see
      // download-picked-torrents.js's identical watchdog for why a flat
      // timer guarantees permanent failure on any file too large to finish
      // within it, no matter how healthy the connection actually is.
